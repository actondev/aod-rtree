...
PRE class Rtree : public RtreeBase<double> {
public:
private:
...
public:
  using RtreeBase = RtreeBase<double>;
  ...
  int search(const Vec &low, const Vec &high,
  ...
  template <typename Fn> void with_transaction(Fn fn) {
...
PRE void QUAL::insert(const Vec &low, const Vec &high, const DATATYPE &data) {
  const Did did = RtreeBase::make_data_id();
...
PRE int QUAL::search(const Vec &low, const Vec &high,
                     std::vector<DATATYPE> &results) const {
  results.clear();
  typename RtreeBase::SearchCb cb = [&results, this](Eid e) {
    const Entry &entry = RtreeBase::get_entry(e);
...
PRE int QUAL::search(const Vec &low, const Vec &high, SearchCb cb) const {
  typename RtreeBase::SearchCb base_cb = [&](Eid e) {
    const Entry &entry = RtreeBase::get_entry(e);
...
PRE int QUAL::search(const Vec &low, const Vec &high, SearchCbExt cb) const {
  ...
  callback_rect.low.resize(RtreeBase::m_dims);
  callback_rect.high.resize(RtreeBase::m_dims);
  typename RtreeBase::SearchCb base_cb = [&](Eid e) {
    const Entry &entry = RtreeBase::get_entry(e);
    const DATATYPE &data = RtreeBase::get_data(entry.data_id);
...
PRE int QUAL::remove(const Vec &low, const Vec &high, Predicate pred) {
  typename RtreeBase::Predicate base_pred = [&](Eid e) {
...
PRE int QUAL::remove(const Vec &low, const Vec &high, PredicateExt pred) {
  Rect rect;
  rect.low.resize(RtreeBase::m_dims);
  rect.high.resize(RtreeBase::m_dims);
